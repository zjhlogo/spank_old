/*!
 * \file NetMgr_Mac_Impl.cpp
 * \date 11-17-2011 12:29:00
 * 
 * 
 * \author zjhlogo (zjhlogo@gmail.com)
 */
#include "NetMgr_Mac_Impl.h"
...
{
	static NetMgr_Mac_Impl s_NetMgr_Mac_Impl;
	return s_NetMgr_Mac_Impl;
}

NetMgr_Mac_Impl::NetMgr_Mac_Impl()
{
	m_nSocket = 0;
...
bool NetMgr_Mac_Impl::Initialize()
{
	m_SendBuffer.Reset();
	return true;
}

void NetMgr_Mac_Impl::Terminate()
{
	Disconnect();
	m_SendBuffer.Reset();
}

void NetMgr_Mac_Impl::Update(float dt)
{
...
bool NetMgr_Mac_Impl::ConnectToServer(const char* ip, ushort port)
{
	uint nIP = 0;
...
bool NetMgr_Mac_Impl::ConnectToServer(uint ip, ushort port)
{
...
void NetMgr_Mac_Impl::Disconnect()
{
	if (m_nSocket != 0)
	{
		close(m_nSocket);
...
bool NetMgr_Mac_Impl::SendNetMessage(INetMsgBase* pMsg)
{
	StreamWriter writer;
...
void NetMgr_Mac_Impl::SetNetMsgHandler(INetMsgHandler* pHandler)
{
	m_pNetMsgHandler = pHandler;
}

INetMsgHandler* NetMgr_Mac_Impl::GetNetMsgHandler()
{
	return m_pNetMsgHandler;
}

bool NetMgr_Mac_Impl::HandlerDataBlock(const void* pData, int nLength)
{
...
void NetMgr_Mac_Impl::ReceiveOnce()
{
	char buffer[1024];
